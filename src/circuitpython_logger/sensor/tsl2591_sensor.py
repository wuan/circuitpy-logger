...
class TSL2591Sensor:
    ...
    def measure(self, data_builder: DataBuilder, measurements: Measurements) -> None:
        lux = self.driver.lux
        data_builder.add(self.name, f"light", "lux", lux)

        infrared = self.driver.infrared
        data_builder.add(self.name, f"light_raw_infrared", "", infrared)

        visible = self.driver.visible
        data_builder.add(self.name, f"light_raw_visible", "", visible)

        full_spectrum = self.driver.full_spectrum
        data_builder.add(self.name, f"light_raw_full_spectrum", "", full_spectrum)
